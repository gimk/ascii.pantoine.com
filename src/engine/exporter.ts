...
/**
 * Generates an embeddable HTML/CSS snippet rendering the ASCII text frame inside a responsive CRT terminal card.
 */
export function generateHtmlEmbed(props: {
  name: string;
  frameText: string;
  cols: number;
  rows: number;
  ...
  const themeColors: Record<string, { bg: string; text: string; glow: string }> = {
    green: { bg: '#040905', text: '#00ff66', glow: 'rgba(0, 255, 102, 0.25)' },
    amber: { bg: '#090602', text: '#ffb000', glow: 'rgba(255, 176, 0, 0.25)' },
    cyan: { bg: '#03080a', text: '#00f0ff', glow: 'rgba(0, 240, 255, 0.25)' },
    monochrome: { bg: '#0a0a0a', text: '#f0f0f0', glow: 'rgba(255, 255, 255, 0.15)' },
    blood: { bg: '#0a0304', text: '#ff3344', glow: 'rgba(255, 51, 68, 0.25)' },
    paper: { bg: '#f0eee6', text: '#151515', glow: 'rgba(0, 0, 0, 0.08)' },
    matrix: { bg: '#040905', text: '#00ff66', glow: 'rgba(0, 255, 102, 0.3)' },
  };

  const colors = themeColors[props.theme || 'green'] || themeColors.green;
  const textColor = props.customThemeColor || colors.text;

  return `<!-- Generated by ASCII Studio (https://ascii.pantoine.com) -->
<!-- Preset: ${props.name} | Resolution: ${props.cols}x${props.rows} -->
<div class="ascii-terminal-card" style="background:${colors.bg}; color:${textColor}; padding:18px 22px; border-radius:6px; border:1px solid ${textColor}33; box-shadow:0 0 20px ${colors.glow}; display:inline-block; max-width:100%; overflow-x:auto;">
  <pre style="font-family:'JuliaMono','JetBrains Mono','Courier New',monospace; font-size:11px; line-height:1.05; letter-spacing:0px; white-space:pre; margin:0; font-variant-ligatures:none; -webkit-font-variant-ligatures:none; text-rendering:geometricPrecision;">${props.frameText}</pre>
  <div style="margin-top:10px; padding-top:8px; border-top:1px solid ${textColor}22; font-family:monospace; font-size:9.5px; opacity:0.75; display:flex; justify-content:space-between;">
    <span>${props.name}</span>
    <a href="https://ascii.pantoine.com" target="_blank" rel="noopener noreferrer" style="color:inherit; text-decoration:underline;">ascii.pantoine.com</a>
  </div>
</div>`;
}

/**
 * Generates a clean Markdown code block with metadata and ASCII Studio credit.
 */
export function generateMarkdownSnippet(props: {
  name: string;
  frameText: string;
  appMode?: AppMode;
  cols?: number;
  ...
  const modeLabel = props.appMode ? `[${props.appMode.toUpperCase()}] ` : '';
  return `<!-- Generated with ASCII Studio (https://ascii.pantoine.com) -->
### ${modeLabel}${props.name}

\`\`\`text
${props.frameText}
\`\`\`

*Rendered with [ASCII Studio](https://ascii.pantoine.com)*`;
}

/**
 * Generates a formatted ASCII plain text frame with header credits.
 */
export function generateAsciiTextFrame(props: {
  name: string;
  frameText: string;
  cols: number;
  rows: number;
  ...
  return `# ==============================================================================
# ASCII ART: ${props.name}
# Generated by ASCII Studio (https://ascii.pantoine.com)
# Mode: ${(props.appMode || 'synth').toUpperCase()} | Grid: ${props.cols} cols x ${props.rows} rows
# ==============================================================================
...
/**
 * Generates clean, mode-specific JSON preset configuration.
 */
export function generateModeJsonPreset(props: {
  appMode?: AppMode;
  name: string;
  type?: 'parametric' | 'custom';
  params?: WaveParams;
  ...
  const base = {
    ...
    crtConfig: props.crtConfig,
  };

  if (props.appMode === 'model') {
...
<head>
  <meta charset="UTF-8">
  <title>${cfg.name} - ASCII Studio</title>
  <style>
